    ...
    return (
        <div className="space-y-8 text-left text-foreground">
            ...
            <div className="p-8 bg-slate-900 text-white rounded-[2rem] shadow-xl space-y-4">
                <h4 className="text-[10px] font-black uppercase text-primary flex items-center gap-2"><UserCheck className="h-4 w-4" /> Identity Node</h4>
                <p className="text-xs text-slate-400">Attach proof of identity for the principal applicant.</p>
                <FileUploadField name="userIdUrl" label="Principal ID / Passport Copy" folder="forensic-main" />
            </div>
        </div>
    );
};
        ...
        <div className="space-y-8 text-left text-foreground">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="p-8 bg-slate-900 text-white rounded-[2rem] shadow-xl flex flex-col justify-center gap-4">
                    ...
                    <p className="text-xs text-slate-400">Upload official CIPC or Trust Founding documentation.</p>
                    <FileUploadField name="registrationDocUrl" label="Registration Doc (CK/CM/Trust)" folder="forensic-entity" />
                </div>
            </div>
        </div>
    );
                ...
                <div className="space-y-6">
                    ...
                    {isVatRegistered && (
                        <FormField control={control} name="vatNumber" render={({ field }) => (
                            <FormItem className="animate-in fade-in slide-in-from-left-2 duration-300"><FormLabel>VAT Number</FormLabel><FormControl><Input {...field} value={field.value || ''} className="bg-white border-2 font-mono" /></FormControl></FormItem>
                        )} />
                    )}
                </div>
                {isVatRegistered && (
...
export function EditClientWizard({ client, onSave, onBack }: { client?: any, onSave: () => void, onBack: () => void }) {
    ...
    const memoizedSteps = useMemo(() => {
        const base = [
            { id: 'capacity', title: 'Main', icon: User, fields: ['applyingCapacity', 'name', 'entityType', 'userIdUrl'] },
        ...
        base.push({ id: 'background', title: 'Step 5: Background', icon: Truck, fields: ['isSelfEmployed', 'yearsInIndustry', 'truckCount', 'trailerCount', 'fleetInventoryUrl'] });
                    ...
                    <CardContent className="p-0">
                        <div className="grid grid-cols-1 md:grid-cols-[260px_1fr]">
                            ...
                            <div className="p-10 space-y-12 bg-white min-h-[600px] text-left text-foreground">
                                {currentStepConfig.id === 'capacity' && <StepMain />}
