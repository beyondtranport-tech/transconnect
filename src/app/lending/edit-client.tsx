...
const StepEntityDetails = () => {
    const { control, watch } = useFormContext<ClientFormValues>();
    const entType = watch('entityType');
    
    return (
        <div className="space-y-8 text-left text-foreground">
            <h3 className="text-2xl font-black font-headline uppercase tracking-tight">Legal Registration</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                    <FormField control={control} name="registrationId" render={({ field }) => (<FormItem><FormLabel>Registration Number</FormLabel><FormControl><Input {...field} value={field.value || ''} placeholder="e.g. 2024/123456/07" className="h-11 border-2 font-mono" /></FormControl></FormItem>)} />
                    <FormField control={control} name="inceptionDate" render={({ field }) => (<FormItem><FormLabel>Inception Date</FormLabel><FormControl><Input {...field} value={field.value || ''} type="date" className="h-11 border-2" /></FormControl></FormItem>)} />
                </div>
...
export function EditClientWizard({ client, onSave, onBack }: { client?: any, onSave: () => void, onBack: () => void }) {
  ...
  const memoizedSteps = useMemo(() => {
    const base = [
        { id: 'main', title: 'Main', icon: User, fields: ['applyingCapacity', 'name', 'entityType', 'userIdUrl'] },
    ...
    base.push({ id: 'background', title: 'Step 5: Background & Footprint', icon: Truck, fields: ['isSelfEmployed', 'yearsInIndustry', 'truckCount', 'trailerCount', 'fleetInventoryUrl'] });
    ...
    base.push({ id: 'infrastructure', title: 'Infrastructure Standing', icon: Building, fields: ['ownsOperatingProperty', 'propertyDetails', 'propertyLiability', 'landlordContact', 'leaseInfo', 'propertyDeedUrl', 'leaseAgreementUrl'] });
  ...
  const onSubmit = async (values: ClientFormValues) => {
    ...
    try {
        const token = await getClientSideAuthToken();
        if (!token) throw new Error("Auth failed.");
        const path = client?.id ? `lendingClients/${client.id}` : `lendingClients`;
        const data = { ...values, status: 'active', updatedAt: serverTimestamp() };
        const response = await fetch(client?.id ? '/api/updateUserDoc' : '/api/addUserDoc', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ [client?.id ? 'path' : 'collectionPath']: path, data }),
        });
        if (!response.ok) throw new Error("Server failure.");
        toast({ title: 'Record Committed' });
        onSave();
    } catch (e: any) {
          ...
          <CardHeader className="bg-slate-900 text-white p-8">
            <div className="flex justify-between items-center text-left">
              <div className="text-left text-white">
                <CardTitle className="text-2xl font-black font-headline uppercase">Forensic Onboarding Terminal</CardTitle>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[260px_1fr]">
              <div className="bg-slate-50 border-r p-6 space-y-2 text-left">
                {memoizedSteps.map((step, i) => (
                  <Button key={step.id} type="button" variant={currentStep === i ? "secondary" : "ghost"} className={cn("w-full justify-start gap-3 h-10 px-3", currentStep === i && "bg-white shadow-sm ring-1 ring-primary/20")} onClick={() => handleStepTransition(i)}>
                    {React.createElement(step.icon, { className: cn("h-4 w-4", currentStep >= i ? "text-primary" : "text-muted-foreground") })}
                    <span className={cn("text-[11px] font-black uppercase", currentStep === i ? "text-primary" : "text-muted-foreground")}>{step.title}</span>
                  </Button>
                ))}
              </div>
              <div className="p-10 space-y-12 bg-white min-h-[600px] text-left text-foreground">
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                ...
                                {watchedValues.vatRegistered && (
                                    <>
                                        <FormField control={methods.control} name="vatNumber" render={({ field }) => (<FormItem className="animate-in fade-in slide-in-from-left-2 duration-300"><FormLabel>VAT Number</FormLabel><FormControl><Input {...field} value={field.value || ''} className="bg-white border-2 font-mono" /></FormControl></FormItem>)} />
                                        ...
                                        {!watchedValues.vatUpToDate && (
                                            <FormField control={methods.control} name="lastVatReturnDate" render={({ field }) => (
                                                <FormItem className="animate-in fade-in slide-in-from-top-2 duration-300 bg-amber-50 p-6 rounded-2xl border border-amber-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div className="space-y-6">
                                <div className="grid grid-cols-3 gap-6">
                                    ...
                                    <FormField control={methods.control} name="directorCount" render={({ field }) => (<FormItem><FormLabel>Directors</FormLabel><FormControl><Input type="number" {...field} className="h-11 border-2 font-bold" /></FormControl></FormItem>)} />
                                    <FormField control={methods.control} name="staffCount" render={({ field }) => (<FormItem><FormLabel>Key Staff</FormLabel><FormControl><Input type="number" {...field} className="h-11 border-2 font-bold" /></FormControl></FormItem>)} />
                                </div>
                ...
                {currentStepConfig.id === 'background' && (
                     <div className="space-y-10 text-left text-foreground">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                            <FormField control={methods.control} name="isSelfEmployed" render={({ field }) => (<FormItem className="flex items-center justify-between p-6 border-2 rounded-3xl bg-white shadow-sm text-left"><div className="space-y-0.5"><FormLabel className="text-sm font-black uppercase text-left">Self-Employed?</FormLabel><FormDescription className="text-xs">Principal employment standing.</FormDescription></div><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl></FormItem>)} />
                            ...
                            <FormItem className="space-y-6 text-left">
                                ...
                                <FormControl>
                                    <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="grid grid-cols-2 gap-6 text-left">
                                        <div className={cn("p-6 border-2 rounded-[2rem] cursor-pointer transition-all shadow-sm", field.value === 'in_house' ? "border-primary bg-primary/5" : "bg-white")}>
                                        ...
                                        <div className={cn("p-6 border-2 rounded-[2rem] cursor-pointer transition-all shadow-sm", field.value === 'external' ? "border-primary bg-primary/5" : "bg-white")}>
                       ...
                       {watchedValues.ownsOperatingProperty ? (
                           ...
                           <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch text-left">
                               <div className="space-y-6 p-8 border-2 border-dashed rounded-3xl bg-slate-50 text-left text-foreground">
                                   <h4 className="text-xs font-black uppercase tracking-widest text-slate-600 flex items-center gap-2 text-left text-foreground"><UserCheck className="h-4 w-4" /> Landlord Node</h4>
                               ...
                               <div className="space-y-6 p-8 border-2 border-dashed rounded-3xl bg-primary/5 text-left text-foreground">
                                   <h4 className="text-xs font-black uppercase tracking-widest text-primary flex items-center gap-2 text-left text-foreground"><FileText className="h-4 w-4" /> Lease Parameters</h4>
                ...
                {currentStepConfig.id === 'review' && (
                    <div className="text-center py-20 space-y-6">
                        ...
                        <p className="text-sm text-muted-foreground max-sm mx-auto text-center leading-relaxed">Verify data integrity before formally committing this node to the registry.</p>
                    </div>
                )}
              </div>
            </div>
    ...
    return (
        <div className="container mx-auto flex min-h-screen items-center justify-center px-4 py-20 text-left bg-slate-50">
            <Suspense fallback={<Loader2 className="animate-spin h-12 w-12 text-primary" />}>
                <EditClientWizard onSave={() => {}} onBack={() => {}} />
            </Suspense>
        </div>
    )
}
