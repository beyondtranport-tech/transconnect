                ...
                <div className="flex-1 overflow-hidden p-6">
                    {view === 'type_selection' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-primary ml-1">Step 1: Select Document Context</Label>
                                    ...
                                    {docTypes.map((type) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1">
                                                <p className="font-bold text-sm leading-none mb-1">{type.label}</p>
                    ...
                    {view === 'upload' && (
                        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500 h-full flex flex-col">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-primary ml-1">Step 2: Review & Execute Scan</Label>
                            <div className="flex-1 relative rounded-2xl border-2 overflow-hidden bg-black shadow-inner">
                                ...
                                {isProcessing && (
                                    <div className="absolute inset-0 bg-black/60 backdrop-blur-md flex flex-col items-center justify-center gap-3">
                                        ...
                                        <p className="text-sm font-black text-white uppercase tracking-widest animate-pulse">Crawling Document Nodes...</p>
                                    </div>
                                )}
                            </div>
                    ...
                    {view === 'result' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500 h-full flex flex-col">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-primary ml-1">Step 3: Forensic Result Audit</Label>
                            <ScrollArea className="flex-1 border-2 border-dashed rounded-2xl bg-slate-50 p-6">
                                <div className="space-y-8">
                                        ...
                                        {Object.entries(result.extraction).map(([key, value]) => {
                                            ...
                                            return (
                                                <div key={key} className="space-y-1">
                                                    <Label className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">{key.replace(/([A-Z])/g, ' $1')}</Label>
                                                    <p className="text-sm font-bold text-slate-900 border-b pb-1 truncate">{String(value)}</p>
                                                </div>
                                            );
                                        })}
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <p className="text-xs text-muted-foreground leading-relaxed italic">"{result.summary}"</p>
                                    </div>
                                </div>
                            </ScrollArea>
