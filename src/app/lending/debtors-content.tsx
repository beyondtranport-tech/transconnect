...
export default function DebtorsContent() {
    ...
    const forceRefresh = useCallback(async () => {
        ...
        try {
            ...
            const result = await performAdminAction(token, 'getLendingData', { collectionName: 'lendingDebtors' });
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = await getClientSideAuthToken();
            if (!token) throw new Error("Authentication failed.");
            await performAdminAction(token, 'deleteLendingPartner', { collection: 'lendingDebtors', partnerId: debtorToDelete.id });
        ...
        <div className="space-y-8 text-left text-foreground">
            <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
                <AlertDialogContent className="text-left text-foreground">
                    <AlertDialogHeader className="text-left">
                        ...
                        <AlertDialogDescription className="text-left">Permanent removal of "{debtorToDelete?.name}" from the registry.</AlertDialogDescription>
                    </AlertDialogHeader>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 text-left">
                <div className="text-left text-foreground">
                    ...
                    <p className="text-muted-foreground mt-1 text-left text-foreground">Specialized portfolio of cessionaries for Factoring and Rights Discounting.</p>
                </div>
            </div>
                    ...
                    <Card className="border-none shadow-xl bg-white text-left text-foreground">
                        <CardHeader className="flex flex-row items-center justify-between border-b bg-muted/10 text-left p-6">
                            ...
                            <div className="flex gap-2 text-left text-foreground">
                                ...
                                <Button variant="outline" size="sm" onClick={() => downloadDataAsCSV(debtors, `debtors-registry-${Date.now()}.csv`)} className="gap-2 text-foreground">
                                    <Download className="h-4 w-4" /> Export
                                </Button>
                                <BulkImportDialog type="lendingDebtor" onComplete={forceRefresh}>
