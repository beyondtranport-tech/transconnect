...
import { PlusCircle, ShieldCheck, Loader2, Save, Trash2, ArrowLeft, ArrowRight, Check } from 'lucide-react';
...
function SecurityWizard({ securityDoc, onBack, onSaveSuccess }: { securityDoc?: any; onBack: () => void; onSaveSuccess: () => void; }) {
    ...
    const isStepValid = (stepIndex: number) => {
        ...
        const fields = step.fields as (keyof SecurityFormValues)[];
                        ...
                        render={({field}) => (
                            <FormItem>
                                ...
                                <Select onValueChange={field.onChange} defaultValue={field.value} disabled={!selectedClientId || areAgreementsLoading}>
                                    <FormControl>
                                        <SelectTrigger><SelectValue placeholder={areAgreementsLoading ? "Loading..." : "Select an agreement..."} /></SelectTrigger>
                                    </FormControl>
            ...
            case 'details':
                 return (
                    <div className="space-y-4">
                        <FormField control={control} name="securityType" render={({ field }) => (<FormItem><FormLabel>Type of Security</FormLabel><FormControl><Input placeholder="e.g., Cession of Debtors" {...field} /></FormControl><FormMessage /></FormItem>)} />
                        <FormField control={control} name="description" render={({ field }) => (<FormItem><FormLabel>Description (Optional)</FormLabel><FormControl><Textarea placeholder="Add any relevant details..." {...field} /></FormControl><FormMessage /></FormItem>)} />
                        <FormField control={control} name="clientCode" render={({ field }) => (<FormItem><FormLabel>Client Code</FormLabel><FormControl><Input placeholder="e.g., STC-001" {...field} /></FormControl><FormMessage /></FormItem>)} />
                    </div>
                );
            case 'status':
                return (
            ...
            case 'review':
                ...
                return (
                    <div className="space-y-2 text-sm">
                        <p><strong>Client:</strong> {clients?.find(c => c.id === values.clientId)?.name}</p>
                        <p><strong>Agreement:</strong> {values.agreementId}</p>
                        <p><strong>Type:</strong> {values.securityType}</p>
                        <p><strong>Status:</strong> {values.docStatus}</p>
                    </div>
                );
            default: return null;
        }
...
export default function SecurityContent() {
    ...
    const columns: ColumnDef<any>[] = useMemo(() => [
        { accessorKey: 'securityType', header: 'Agreement Type' },
