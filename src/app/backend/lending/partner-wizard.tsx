...
// --- Step Definitions ---
const steps = [
    { id: 'main', name: 'Main Details', fields: ['name', 'type', 'category', 'status', 'regId', 'vatNo'] },
...
const defaultValues: Partial<PartnerFormValues> = {
  name: '', type: '', category: '', language: '', regId: '', status: 'draft',
                    ...
                    render={({ field }) => (
                        <FormItem className="flex items-center space-x-2 mb-4">
                            <FormControl><Checkbox checked={field.value} onCheckedChange={(checked) => {
                                ...
                                if (checked) {
                                    setValue('postalStreet', watch('physicalStreet') || '');
                                    setValue('postalSuburb', watch('physicalSuburb') || '');
                                    setValue('postalCity', watch('physicalCity') || '');
                                    setValue('postalPostCode', watch('physicalPostCode') || '');
                                    setValue('postalProvince', watch('physicalProvince') || '');
                                }
                            }} /></FormControl>
...
export default function PartnerWizard({ partnerData, partnerType, onBack, onSaveSuccess }: { partnerData?: any; partnerType: string; onBack: () => void; onSaveSuccess: () => void; }) {
    ...
    const isStepValid = (stepIndex: number) => {
        ...
        const fields = step.fields as (keyof PartnerFormValues)[];
