...
if (!getApps().length) {
    try {
        ...
        if (!serviceAccountString) {
            // This is not a critical error in development if not all server actions are used.
            // A more specific error will be thrown by functions that need the SDK.
        } else {
        ...
        console.error("Firebase Admin SDK initialization failed during startup. Server actions may not work.", e);
    }
} else {
  adminApp = getApp();
}
