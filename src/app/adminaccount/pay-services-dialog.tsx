...
import { useCollection, useFirestore, useMemoFirebase, getClientSideAuthToken, useDoc } from '@/firebase';
...
export default function PayServicesDialog({ company, onPaymentSuccess }: { company: any, onPaymentSuccess: () => void }) {
    ...
    const pendingPaymentsQuery = useMemoFirebase(() => {
        ...
        return query(collection(firestore, `companies/${company.id}/walletPayments`), where('status', '==', 'pending'));
    }, [firestore, company]);
    ...
    const handlePay = async (payment: any) => {
        if (!company || company.walletBalance < payment.amount) {
                    ...
                    {arePaymentsLoading ? (
                        ...
                        payments.map(payment => (
                             <div key={payment.id} className="p-4 border rounded-lg space-y-2">
                                ...
                                <p className="text-xs text-muted-foreground">Due: {format(payment.createdAt.toDate(), "dd MMM yyyy")}</p>
                                <Button 
                                    className="w-full mt-2" 
                                    onClick={() => handlePay(payment)}
                                    disabled={!!isProcessing || company.walletBalance < payment.amount}
                                >
                                    {isProcessing === payment.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Wallet className="mr-2 h-4 w-4"/>}
                                    {company.walletBalance < payment.amount ? 'Insufficient Balance' : 'Pay Now'}
                                </Button>
                            </div>
                        ))
                    ) : (
