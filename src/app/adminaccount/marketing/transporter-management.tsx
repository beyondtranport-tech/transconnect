...
import { Loader2, PlusCircle, Truck, Edit, Trash2, Send, CheckCircle, Users, MailCheck, MailQuestion } from 'lucide-react';
...
function TransporterFormDialog({ open, onOpenChange, partner, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; partner?: any; onSave: () => void; }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      else form.reset({ firstName: '', lastName: '', email: '', phone: '', companyName: '', address: '', status: 'active', type: 'transporter' });
    }
  }, [open, partner, form]);
  ...
  return (
    <>
      ...
      <AlertDialog open={dialog.type === 'delete'} onOpenChange={(o) => !o && setDialog({ type: null })}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This will permanently delete transporter "{dialog.data?.firstName} {dialog.data?.lastName}".</AlertDialogDescription></AlertDialogHeader>
