...
function InvestorDialog({ open, onOpenChange, partner, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; partner?: any; onSave: () => void; }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      else form.reset({ firstName: '', lastName: '', email: '', phone: '', companyName: '', status: 'active', type: 'investor' });
    }
  }, [open, partner, form]);
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4 max-h-[80vh] overflow-y-auto pr-2">
            ...
            <DialogFooter className="pt-4 border-t">
              <Button type="submit" disabled={isLoading}>{isLoading ? <Loader2 className="animate-spin h-4 w-4" /> : null} Save Investor</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
...
export default function InvestorManagement() {
  ...
  const forceRefresh = useCallback(async () => {
    ...
    try {
      const token = await getClientSideAuthToken();
      if (!token) return;
      const res = await performAdminAction(token, 'getPartnersByType', { type: 'investor' });
