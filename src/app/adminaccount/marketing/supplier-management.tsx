                ...
                <CardContent className="p-0">
                    {group.map(lead => (
                        <div key={lead.id} className={cn("flex items-start gap-4 p-4 border-b last:border-b-0", selections[groupIndex] === lead.id ? "bg-primary/5" : "")}>
                            <Checkbox
                                id={`supplier-${groupIndex}-${lead.id}`}
                                checked={selections[groupIndex] === lead.id}
                                onCheckedChange={() => setSelections(prev => ({ ...prev, [groupIndex]: lead.id }))}
                            />
                            <label htmlFor={`supplier-${groupIndex}-${lead.id}`} className="text-sm cursor-pointer w-full">
...
export default function SupplierManagement() {
  ...
  const forceRefresh = useCallback(async () => {
    ...
    try {
      ...
      const [res, staffRes] = await Promise.all([
        performAdminAction(token, 'getPartnersByType', { type: 'supplier', t }),
  ...
  const handleCopyBccList = () => {
    ...
    toast({ title: "BCC List Copied!", description: `${emails.length} email addresses copied.` });
  };

  const handleExportCsv = () => {
    if (filteredSuppliers.length === 0) return;
    
    const headers = ["Supplier Name", "Contact Person", "Email", "Phone", "Status", "Assignee", "Category"];
    const rows = filteredSuppliers.map(t => [
  ...
  return (
    <>
      ...
      <AlertDialog open={dialog.type === 'delete'} onOpenChange={(o) => !o && setDialog({ type: null })}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Delete Supplier Lead?</AlertDialogTitle><AlertDialogDescription>Delete "{dialog.data?.companyName || 'this record'}"?</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="space-y-6">
        <CardHeader className="px-0 pt-0 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <BulkImportDialog type="supplier" onComplete={forceRefresh}><Button variant="outline">Import JSON</Button></BulkImportDialog>
