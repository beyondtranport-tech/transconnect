...
import { Loader2, PlusCircle, Handshake, Edit, Trash2, Send, CheckCircle, Users, MailCheck, MailQuestion, Clock } from 'lucide-react';
...
function PartnerDialog({ open, onOpenChange, partner, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; partner?: any; onSave: () => void; }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      else form.reset({ firstName: '', lastName: '', email: '', phone: '', companyName: '', address: '', status: 'active', type: 'partner' });
    }
  }, [open, partner, form]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>Enter the partner details.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
  ...
  return (
    <>
      ...
      <AlertDialog open={dialog.type === 'delete'} onOpenChange={(o) => !o && setDialog({ type: null })}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This will permanently delete partner "{dialog.data?.firstName} {dialog.data?.lastName}".</AlertDialogDescription></AlertDialogHeader>
