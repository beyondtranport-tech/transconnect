...
function PartnerDialog({ open, onOpenChange, partner, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; partner?: any; onSave: () => void; }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      else form.reset({ firstName: '', lastName: '', email: '', phone: '', mobile: '', contactPerson: '', companyName: '', status: 'active', type: 'partner' });
    }
  }, [open, partner, form]);
...
export default function PartnerManagement() {
  ...
  const fetchData = useCallback(async (isLoadMore: boolean = false) => {
    ...
    try {
      ...
      const payload: any = { type: 'partner' };
      ...
      <div className="space-y-6 text-left">
        <CardHeader className="px-0 pt-0 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="text-left">
                ...
                <CardDescription>Managed view of key partners (500 per page).</CardDescription>
            </div>
