...
function generateDiscoveryPrompt(category: string, startSeq: number = 1) {
    return `ACT AS AN ELITE INDUSTRIAL FORENSIC INVESTIGATOR AND SUPPLY CHAIN ANALYST. 
RETURN ONLY A RAW JSON ARRAY. NO MARKDOWN. NO CODE BLOCKS. NO CONVERSATION.

CRITICAL INTEGRITY SHIELD: 
DO NOT RETURN MOCK OR PLACEHOLDER DATA. 
YOU MUST PERFORM A LIVE SEARCH FOR "${category} facilities in South Africa" on Google Maps, LinkedIn, and industrial logistics directories.
VERIFY THE FACILITY HAS A PHYSICAL OPERATIONAL PRESENCE.
...
3. HUMAN IDENTITY: Find the ACTUAL FULL NAME of the Facility Manager, MD, or Owner.
...
[
  {
    ...
    "notes": "Summary of storage capacity, temperature range (if applicable), and security features."
  }
]`;
}
...
const DiscoveryTab = ({ category, currentCount }: { category: string, currentCount: number }) => {
    ...
    const handleCopy = async () => {
        ...
        setIsCopied(true);
        toast({ title: "Forensic Prompt Ready", description: "Targeted at storage infrastructure." });
        ...
        <div className="grid md:grid-cols-2 gap-6 text-left">
            <div className="space-y-4 text-left text-foreground">
                ...
                <Alert className="bg-primary/5 border-primary/20 text-left">
                    ...
                    <AlertTitle className="font-bold text-left text-foreground">Infrastructure Mapping</AlertTitle>
                    <AlertDescription className="text-xs text-left text-muted-foreground">
                        Scouting for warehouse operators who can list excess capacity in the Mall. Focus on facilities with verified security and handling equipment.
                    </AlertDescription>
                </Alert>
            ...
            <Tabs defaultValue="Cold Storage" className="w-full text-left">
                <CardHeader className="px-0 pt-0 text-left text-foreground">
                    <CardTitle className="flex items-center gap-2 text-foreground font-black font-headline text-left">
                        ...
                        Warehouse Discovery Engine
                    </CardTitle>
                    <CardDescription className="text-muted-foreground text-left text-foreground">Map South African storage capacity and 3PL nodes.</CardDescription>
                </CardHeader>
