...
import TransactionAllocation from "@/app/backend/reconciliation/transaction-allocation";
import { getClientSideAuthToken, useCollection, useFirestore, useMemoFirebase } from "@/firebase";
import demoStatementData from '@/lib/demo-admin-statement.json';
...
function ReconciliationDashboard() {
    ...
    useEffect(() => {
        if (useDemo) {
            setProcessingData(demoStatementData);
             toast({ title: "Admin Demo Statement Loaded", description: "Sample business transactions are ready for reconciliation." });
        } else {
            if (processingData && processingData.statementName === 'demo-admin-statement.csv') {
                 setProcessingData(null);
            }
        }
    ...
    const handleManualAdjustment = () => {
        ...
        toast({
            ...
            description: "You can now add manual transactions below for the business operating account.",
        });
        setProcessingData({
        ...
        <div className="w-full space-y-8">
            <Card>
                <CardHeader>
                    <div className="flex justify-between items-start">
                        <div>
                            <CardTitle>Business Account Reconciliation</CardTitle>
                            <CardDescription>
                                Reconcile the main business operating account statement.
                            </CardDescription>
                        </div>
                ...
                <CardContent>
                    <div className="flex items-center space-x-2 mb-4 p-4 border rounded-md bg-muted/50">
                        ...
                        <Label htmlFor="demo-mode" className="cursor-pointer">Use Admin Demo Statement</Label>
                    </div>
                    {!processingData && (
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Review all previously completed reconciliation reports for the business account.</CardDescription>
                </CardHeader>
                <CardContent>
                    {isLoadingHistory ? (
