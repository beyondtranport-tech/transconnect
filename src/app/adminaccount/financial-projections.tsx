...
function calculateProjections(inputs: FormValues) {
    const { forecastMonths, salesRoadmap, targets, budget } = inputs;
    ...
    const totals = useMemo(() => {
        return projections.reduce((acc, curr) => {
            Object.keys(curr).forEach(key => {
                if (key !== 'month' && !key.startsWith('cumulative')) {
                    acc[key as keyof typeof acc] = (acc[key as keyof typeof acc] || 0) + (curr[key as keyof typeof curr] as number);
                }
            });
            // Set cumulative fields to the last value
            acc.cumulativePartnerMembers = curr.cumulativePartnerMembers;
            acc.cumulativeTotalMembers = curr.cumulativeTotalMembers;
            return acc;
        }, {} as any);
    }, [projections]);
